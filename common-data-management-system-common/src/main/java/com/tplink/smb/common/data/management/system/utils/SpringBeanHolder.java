...
public class SpringBeanHolder implements ApplicationContextAware, DisposableBean {
   ...
   * @param callBack 回调函数
   */
  public static synchronized void addCallBacks(SpringBeanHolder.CallBack callBack) {
    if (addCallback) {
  ...
  public static List<String> getAllServiceBeanName() {
    return new ArrayList<>(
        Arrays.asList(applicationContext.getBeanNamesForAnnotation(Service.class)));
  }
    ...
    if (SpringBeanHolder.applicationContext != null) {
      log.warn(
          "SpringContextHolder中的ApplicationContext被覆盖, 原有ApplicationContext为:"
              + SpringBeanHolder.applicationContext);
    }
