...
import com.tplink.smb.common.data.management.system.gen.repository.ApprovalRecordRepository;
import com.tplink.smb.common.data.management.system.gen.repository.CurrentApprovalStatusRepository;
...
public class ApplicationFormServiceImpl implements ApplicationFormService {
  ...
  private void enrichApprovalInfo(ApplicationFormDto dto, Integer applicationFormId) {
    ...
    List<ApprovalRecord> approvalRecords =
        approvalRecordRepository.findByApplicationFormIdOrderByRoundAscStepOrderAscApprovedAtAsc(
            applicationFormId);
    ...
    if (maxRound > 0) {
      // 只处理最新轮次的审批记录
      List<ApprovalRecord> latestRoundRecords =
          approvalRecords.stream()
  ...
  public List<PendingApprovalDto> getPendingApprovals(String approverUserName) {
    List<CurrentApprovalStatus> pendingStatuses =
        currentApprovalStatusRepository.findByApproverUserNameAndStatusOrderByApplicationFormId(
            approverUserName, CurrentApprovalStatus.STATUS_PEDING);

    List<PendingApprovalDto> result = new ArrayList<>();
    for (CurrentApprovalStatus status : pendingStatuses) {
      Optional<ApplicationForm> formOpt =
          applicationFormRepository.findById(status.getApplicationFormId());
  ...
  public PageResult<PendingApprovalDto> getPendingApprovals(
    ...
    List<CurrentApprovalStatus> pendingStatuses =
        currentApprovalStatusRepository.findByApproverUserNameAndStatusOrderByApplicationFormId(
            criteria.getApproverUserName(), CurrentApprovalStatus.STATUS_PEDING);

    // Step 2: 过滤有效的申请单（状态必须为已提交或待审批）
    List<PendingApprovalDto> result =
        pendingStatuses.stream()
            .map(
                status -> {
                  Optional<ApplicationForm> formOpt =
                      applicationFormRepository.findById(status.getApplicationFormId());
  ...
  public PageResult<ApplicationFormDto> getApprovedApplications(
    ...
    List<ApprovalRecord> records =
        approvalRecordRepository.findByApproverUserNameAndApprovalStatusInOrderByApprovedAtDesc(
            criteria.getApproverUserName(),
            Arrays.asList(ApprovalRecord.STATUS_APPROVED, ApprovalRecord.STATUS_REJECTED));
    ...
    for (ApprovalRecord record : records) {
      if (!latestApprovals.containsKey(record.getApplicationFormId())
          || record
              .getApprovedAt()
              .after(latestApprovals.get(record.getApplicationFormId()).getApprovedAt())) {
    ...
    for (ApprovalRecord record : latestApprovals.values()) {
      Optional<ApplicationForm> formOpt =
          applicationFormRepository.findById(record.getApplicationFormId());
      if (formOpt.isPresent()) {
    ...
    // 应用查询条件过滤
    List<ApplicationFormDto> filteredResult =
        result.stream()
            .filter(
                dto -> {
                  boolean match = true;
                  ...
                  if (criteria.getUuid() != null && !criteria.getUuid().isEmpty()) {
                    match =
                        match
                            && dto.getUuid() != null
                            && dto.getUuid().contains(criteria.getUuid());
                  }

                  // 根据申请人姓名过滤
                  if (criteria.getApplicantUserName() != null
                      && !criteria.getApplicantUserName().isEmpty()) {
                    match =
                        match
                            && dto.getApplicantUserName() != null
                            && dto.getApplicantUserName().contains(criteria.getApplicantUserName());
                  }
                  ...
                  if (criteria.getApplicationType() != null) {
                    match =
                        match
                            && dto.getApplicationType() != null
                            && dto.getApplicationType().equals(criteria.getApplicationType());
                  }
                  ...
                  if (criteria.getApplicationDataType() != null) {
                    match =
                        match
                            && dto.getApplicationDataType() != null
                            && dto.getApplicationDataType()
                                .equals(criteria.getApplicationDataType());
                  }

                  // 根据申请单标题过滤
                  if (criteria.getApplicationTitle() != null
                      && !criteria.getApplicationTitle().isEmpty()) {
                    match =
                        match
                            && dto.getApplicationTitle() != null
                            && dto.getApplicationTitle().contains(criteria.getApplicationTitle());
                  }
  ...
  public void approveApplication(
    ...
    List<CurrentApprovalStatus> currentStatuses =
        currentApprovalStatusRepository.findByApplicationFormIdAndRound(
            applicationFormId, currentRound);

    CurrentApprovalStatus currentStatus =
        currentStatuses.stream()
            .filter(
                s ->
                    s.getApproverUserName().equals(approverUserName)
    ...
    if (Objects.equals(approvalStatus, ApprovalRecord.STATUS_REJECTED)) {
      ...
      Integer pendingCount =
          currentApprovalStatusRepository.countPendingByApplicationFormIdAndRoundAndStepOrder(
