...
import com.tplink.smb.common.data.management.system.repository.S3StorageRepository;
...
public class S3StorageServiceImpl implements S3StorageService {
  ...
  public List<S3Storage> queryAll(S3StorageQueryCriteria criteria) {
    return s3StorageRepository.findAll(
        (root, criteriaQuery, criteriaBuilder) ->
            QueryHelp.getPredicate(root, criteria, criteriaBuilder));
  }
  ...
  public S3Storage upload(MultipartFile file) {
    ...
    PutObjectRequest putObjectRequest =
        PutObjectRequest.builder().bucket(amzS3Config.getDefaultBucket()).key(filePath).build();
    // 创建 S3Storage 实例
  ...
  private boolean createBucket(String bucketName) {
    try {
      ...
      CreateBucketRequest bucketRequest =
          CreateBucketRequest.builder().bucket(bucketName).acl(BucketCannedACL.PRIVATE).build();
      ...
      waiterResponse
          ...
          .ifPresent(
              response ->
                  log.info(
                      "存储桶 '{}' 创建成功，状态: {}", bucketName, response.sdkHttpResponse().statusCode()));
